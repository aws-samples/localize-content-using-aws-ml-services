...
def handler(event, context):
    ...
    response = client.start_speech_synthesis_task(Engine=polly_engine, LanguageCode=polly_language_code, OutputFormat="json",
                                                  SampleRate="22050", Text=polly_text, VoiceId=polly_voice_id,
                                                  TextType="text",
                                                  OutputS3BucketName=bucket, SpeechMarkTypes=['word'],
                                                  OutputS3KeyPrefix=f'{uid}/synthesisOutput/{target_language_code}-polly-speech-marks')
